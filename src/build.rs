...
pub struct Build {
    ...
    #[argh(option)]
    /// path to Feather server executable, so you wouldn't
    /// need to copy plugin file to plugins folder
...
impl Build {
    ...
    pub fn target_path(&self, plugin_meta: &PluginMetadata, module_path: PathBuf) -> anyhow::Result<PathBuf> {
        if let Some(path) = &self.server_path {
            let mut plugin_dir = PathBuf::from(path);
            plugin_dir.pop();
            plugin_dir.push("plugins");
            fs::create_dir_all(plugin_dir.clone())?;

            let mut plugin_path = plugin_dir;
            plugin_path.push(format!("{}.plugin", plugin_meta.identifier));
            Ok(plugin_path)
        } else {
            Ok(module_path
                .parent()
                .unwrap()
                .join(format!("{}.plugin", plugin_meta.identifier)))
        }
    }
...
pub fn build(args: Build) -> anyhow::Result<()> {
    ...
    let target_path = args.target_path(&meta, module_path)?;
